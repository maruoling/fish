// requestAnimationFrame()的兼容性封装：
// 这段代码先检查了window.requestAnimationFrame函数的定义是否存在。
//如果不存在，就遍历已知的各种浏览器实现并替代该函数。
// 如果还是找不到一个与浏览器相关的实现，它最终会采用基于JavaScript定时器的动画以每秒60帧的间隔调用setTimeout函数。
...
function calLength2(x1, y1, x2, y2) {
	return Math.pow(x1 - x2, 2) + Math.pow(y1 - y2, 2);//pow() 方法返回 x 的 y 次幂。相当于(x1-x2)^2+(y1-y2)^2
}
